...
To control the vehicle, you can use a video game joystick by launching ```joystick.launch.py```. This launch file starts the joystick_controller node, designed specifically for compatibility with an Xbox One joystick, to interface with the ```desired_velocity``` and ```desired_angle``` topics.
...
max_steering_angular_velocity: 1.570796 # Maximum angular velocity for steering adjustment [rad/s]
