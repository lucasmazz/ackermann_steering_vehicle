...
The vehicle can be controlled by publishing the steering angle in radians and speed in meters per second to the respective topics:
...
To control the vehicle, you can use a video game joystick by launching ```joystick.launch.py```. This launch file starts the joystick_controller node, designed specifically for compatibility with an Xbox One joystick, to interface with the ```desired_speed``` and ```desired_angle``` topics.
...
max_steering_velocity: 1.570796 # Maximum angular velocity for steering adjustment [rad/s]
max_steering_effort: 1.0 # Maximum torque for steering [Nm]
max_velocity: 0.5 # Maximum linear velocity of the vehicle [m/s]
max_effort: 10.0 # Maximum torque for wheel drive [Nm]
