  ...
  wheel_steering_angle_{0.0, 0.0}
{
  ...
  // Publishers
  position_publisher_ =
    create_publisher<std_msgs::msg::Float64MultiArray>("/forward_position_controller/commands", 10);

  velocity_publisher_ =
    create_publisher<std_msgs::msg::Float64MultiArray>("/forward_velocity_controller/commands", 10);
...
std::pair<double, double> VehicleController::ackermann_steering_angle(const double angle)
{
  ...
  if (abs(angle) > 1e-3) {
    const double sin_angle = sin(abs(angle));
    const double cos_angle = cos(abs(angle));
...
std::pair<double, double> VehicleController::rear_differential_velocity(const double velocity,
                                                                        const double angle)
{
  double left_wheel_velocity{velocity};
  double right_wheel_velocity{velocity};

  // Steering angle is not zero nor too small
  if (abs(angle) > 1e-3) {
    // Calculate turning radius and angular velocity
    const double turning_radius = wheel_base_ / tan(abs(angle));        // [m]
    const double vehicle_angular_velocity = velocity / turning_radius;  // [rad/s]
    ...
    const double max_wheel_velocity = std::max(abs(left_wheel_velocity), abs(right_wheel_velocity));
...
void VehicleController::timer_callback()
{
  ...
  const auto elapsed_time{(current_time - last_message_time_).nanoseconds()};
...
void VehicleController::desired_angle_callback(const std_msgs::msg::Float64::SharedPtr msg)
{
  ...
  const auto wheel_angles{ackermann_steering_angle(steering_angle_)};
...
void VehicleController::desired_velocity_callback(const std_msgs::msg::Float64::SharedPtr msg)
{
  ...
  const auto wheel_velocity{rear_differential_velocity(velocity_, steering_angle_)};
