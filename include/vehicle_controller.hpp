...
public:
  VehicleController(const double timer_period = 1e2, const double timeout_duration = 1e9);
...
private:
   ...
   * 
   * @param angle The steering angle of the vehicle [rad].
   * - Zero for neutral (no steering)
   * - Negative for steering right
   * - Positive for steering left
   * 
   * @return A pair of left and right steering angles [rad].
   */
  std::pair<double, double> ackermann_steering_angle(const double angle);
   ...
   * 
   * @param velocity The linear velocity of the vehicle [m/s].
   ...
   */
  std::pair<double, double> rear_differential_velocity(const double velocity, const double angle);
